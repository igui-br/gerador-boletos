...
var GeradorDeBoleto = (function() {
  ...
  GeradorDeBoleto.prototype.gerarPDF = function(args, callback) {
    ...
    boletos.forEach(function escreveOsDadosDoBoleto(boleto, indice) {
      ...
      const pixQRCode = boleto.getPixQRCode();
      ...
      SVGtoPDF(pdf, new Boleto(linhaDigitavel).toSVG(), margemDoSegundoBlocoLayout, 100, {
        lineBreak: false,
        width: 340,
        align: 'left'
      });
      ...
      //Pix QRCode
      if (pixQRCode) {
        pdf.image(`data:image/png;base64,${pixQRCode}`, args.ajusteX + tituloDataProcessamento - 5, args.ajusteY + tituloDaSetimaLinhaLateral + 8, {
          fit: [100, 100]
        });
        pdf.font('negrito')
           .fontSize(args.tamanhoDaFonteDoTitulo)
           .text('Pague com PIX', args.ajusteX + tituloValorMoeda - 25, args.ajusteY + tituloDaDecimaPrimLinhaLateral + 8, {
              lineBreak: false,
              width: 100,
              align: 'left'
        });
